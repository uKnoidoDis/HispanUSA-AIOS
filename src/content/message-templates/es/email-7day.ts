...
const email7DayEs: EmailTemplate = {
  subject: (vars: TemplateVars): string =>
    `Recordatorio: Su cita es en 7 días — documentos pendientes`,

  html: (vars: TemplateVars, checklistsHtml: string): string => `
<!DOCTYPE html>
<html lang="es">
<head><meta charset="UTF-8"><meta name="color-scheme" content="light only"><meta name="supported-color-schemes" content="light only"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="margin:0;padding:0;background-color:#f5f5f5;font-family:Arial,sans-serif;color-scheme:light only;supported-color-schemes:light only;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:8px;overflow:hidden;max-width:600px;">
        ...
        <tr><td style="padding:40px;">
          ...
          <p style="font-size:15px;color:#374151;margin:0 0 24px;">
            Su cita con HispanUSA es en <strong>7 días</strong>. Le recordamos que aún no hemos recibido sus documentos.
            Para garantizar que su cita se realice sin contratiempos, por favor envíelos lo antes posible.
          </p>
          <div style="background-color:#fefce8;border-left:4px solid #ca8a04;border-radius:4px;padding:16px 20px;margin-bottom:32px;">
            <p style="margin:0;font-size:14px;color:#ca8a04;font-weight:bold;">⚠️ ACCIÓN REQUERIDA</p>
            <p style="margin:8px 0 0;font-size:15px;color:#1f2937;">
              📅 Su cita: <strong>${vars.date}</strong>
            </p>
          </div>
          ${checklistsHtml}
          <div style="background-color:#f9fafb;border-radius:6px;padding:20px;margin-top:32px;text-align:center;">
            ...
            <p style="margin:8px 0 0;font-size:14px;color:#6b7280;">o llame al <strong>954-934-0194</strong></p>
          </div>
        </td></tr>
